...
- **月份连续滚动**：切换月份时，日期按真实时间顺序整体上下滚动，不再像两张日历直接替换。
- **快速切换更稳定**：连续点击或按键时会按顺序完成月份切换，并尽量保留原本选择的日号。
- **顶部控件不再变灰**：月份滚动时，年份、“今天”、切换箭头和外观按钮始终保持正常显示。
- **年份与月份选择器**：年份和月份都使用 3 列×4 行布局，并复用日期的悬停、选中和蓝色反馈。
- **当天完成可以看清**：点击对勾后，卡片先显示完成状态和蓝色反馈，再在原位折叠收起，不会瞬间消失。
- **逾期归属更准确**：今天页的历史逾期任务仍会缩成橙色圆环回到原日期；离开今天后不再重复显示全局逾期区。
- **版本身份可见**：正式版显示 `2.0.5 (25) · V5`，源码预览额外显示修订号和预览标记。
...
- 当天待办完成时不飞行：它会先在原位显示对勾、删除线和蓝色反馈，再稳定地折叠收起。
- 月份切换复用一条真实日期轨道，整个月历沿上下方向连续滚动，不使用透明度替换。
